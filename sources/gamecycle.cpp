...
int map_x=0,map_y=0;	/* En que habitaci�n dentro del mundo		*/ 
int pers_x=0,pers_y=0;	/* Posici�n del personaje en la habitaci�n	*/ 
int pers_pos=0;
...
bool previous_x_collision=false;	/* Hubo una colisi�n en X en el frame anterior? */ 
bool previous_y_collision=false;	/* Hubo una colisi�n en Y en el frame anterior? */ 
int hit_time=0;	/* Contador para saber cuando pueden da�arnos de nuevo	*/ 
int character=0;	/* Personaje seleccionado	*/ 
bool live_character[2]={true,true};	/* Si los personajes est�n vivos o muertos */ 
...
bool room_demonroom=false,old_room_demonroom=false;	/* Indica si es la habitaci�n de un monstruo. */ 
...
/* Teclas: */ 
SDL_Scancode UP_KEY=SDL_SCANCODE_Q,DOWN_KEY=SDL_SCANCODE_A,LEFT_KEY=SDL_SCANCODE_O,RIGHT_KEY=SDL_SCANCODE_P;
SDL_Scancode SWORD_KEY=SDL_SCANCODE_SPACE,WEAPON_KEY=SDL_SCANCODE_M,ITEM_KEY=SDL_SCANCODE_F1,PAUSE_KEY=SDL_SCANCODE_F2;
SDL_Scancode last_word[16];
...
//				set_palete((BYTE *)tiles_bmp->get_palete());
			...
			/* Gestiona vistas: */ 
			if (keyboard[SDL_SCANCODE_PAGEUP] && zoom<640) zoom+=8;
			if (keyboard[SDL_SCANCODE_PAGEDOWN] && zoom>320) zoom-=8;
	...
	case 6:	if (!old_keyboard[ITEM_KEY] && !old_keyboard[SDL_SCANCODE_RETURN]) {
			...
			break;
	case 7:	/* DENTRO DE ALG�N PASAJE, TIENDA, ETC.: */ 
			if (SUBSTATE==0) {
				SUBSTATE=1;
				currently_selecting=0;
				passage_state=0;
	...
	case 10:/* REDEFINIENDO TECLAS!!! */ 
			{
				...
				if (SUBSTATE>=1) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)UP_KEY));
				...
				if (SUBSTATE>=2) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)DOWN_KEY));
				...
				if (SUBSTATE>=3) {  
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)LEFT_KEY));
				...
				if (SUBSTATE>=4) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)RIGHT_KEY));
				...
				if (SUBSTATE>=5) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)SWORD_KEY));
				...
				if (SUBSTATE>=6) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)WEAPON_KEY));
				...
				if (SUBSTATE>=7) {
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)ITEM_KEY));
				...
				if (SUBSTATE>=8) { 
					sprintf(tmp,SDL_GetKeyName((SDL_Scancode)PAUSE_KEY));
				...
				for(i=0;!found && i<SDL_NUM_SCANCODES;i++) {
					if (keyboard[i] && !old_keyboard[i]) {
						...
						case 0:UP_KEY=(SDL_Scancode)i;
							   ...
							   found=true;
							   break;
						case 1:DOWN_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 2:LEFT_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 3:RIGHT_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 4:SWORD_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 5:WEAPON_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 6:ITEM_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						case 7:PAUSE_KEY=(SDL_Scancode)i;
							   SUBSTATE++;
							   found=true;
							   break;
						} /* switch */ 
	...
	case 11:/* MENSAJE DE PERSONAJE MUERTO: */ 
			{
				...
				if (live_character[1] ||
					...
					if (SUBSTATE>=120) { 
						...
						/* Calcular la nueva posici�n del personaje en el mapa: */ 
