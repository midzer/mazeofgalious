...
void ActualizeLevers(int dx,int dy)
{
	...
	/* Corregir la posici�n de la espada: */ 
	if (sword) {
		...
		case T_DOOR:
			{
				...
				if (pers_x>=(object[i].x-4*TILE_UNIT) && pers_x<=(object[i].x+4*TILE_UNIT) &&
					pers_y==object[i].y && keyboard[DOWN_KEY] && to_enter_cut==0 &&
					pers_state!=S_DEATH) {
...
void GameRoomEvents()
{
	...
	if (map==2 && map_x==5 && map_y==1) {
		/* Puente que aparece con la mu�eca: */ 
	...
	if (map==0 && map_x==world10_door_x && map_y==world10_door_y && next_world==10) {
		...
		if (!found) {
			/* Hacer que la puerta est� detr�s de todo: */ 
...
{	
	...
	if (n_arrows>0 && 
		...
		case 0:/* FLECHAS NORMALES: */ 
			/* Al tener el "ROD", el n�mero de flechas no disminuye: */ 
